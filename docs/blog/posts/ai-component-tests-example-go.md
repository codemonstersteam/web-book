...
Шесть подходов по 45 минут и один в 30 (300 минут). Вместе с AI я создал [заготовку сервиса на Go](https://git.codemonsters.team/guides/wallet-balance) и доработал GitLab CI — всего за 5 часов. Планировал работать неделю по 90 минут в день, рассчитывал на 300 минут напряжённой работы. Не учёл риски, непредвиденные обстоятельства и стресс, который замедляет мышление.
...
Компонентные тесты на Java, использующие Cucumber, AssertJ.
Код тестов простой и понятный, нет лишних абстракций.

### Как локально валидировать проект:  
...
### Как зупускаются компонентные тесты:
...
Это скрипт используется в CI и гарантированно завершается с кодом ошибки, если тесты не прошли.
