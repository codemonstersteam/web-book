...
description:
  Пошаговый разбор: как построить Go-сервис авторизации без паролей
  вместе с ИИ-агентом — от пустой папки до OpenAPI-спеки.
categories:
  - code
  - ai
  - go
...
![Claude Code extension in VS Code](https://marketplace.visualstudio.com/items?itemName=anthropic.claude-code)
...
Три команды — и агент знает всё: правила разработки, что строим, на каком шаге находимся.
...
```
создай backlog, который будет содержать план работы в соответствии с AGENTS.md
```

Агент создаёт `backlog.md` и предложит перейти к первому шагу. Напиши:
